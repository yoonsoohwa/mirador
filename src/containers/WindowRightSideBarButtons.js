...
import { WindowRightSideBarButtons } from '../components/WindowRightSideBarButtons';

/**
 * mapDispatchToProps - used to hook up connect to action creators
 * @memberof WindowRightSideButtons
 * @private
 */
...
function hasLayers(canvases, getMiradorCanvas) {
  return (
    canvases &&
    canvases.some((c) => getMiradorCanvas(c).imageResources.length > 1)
  );
}
...
function hasAnnotations(canvases, getMiradorCanvas) {
  return (
    ...
    canvases.some((c) => {
      ...
      return (
        canvas.annotationListUris.length > 0 ||
...
function hasSearchResults(state, { windowId }) {
  ...
  const companionWindowId = getCompanionWindowsForPosition(state, {
    position: 'left',
    windowId,
  })?.[0]?.id;
...
const mapStateToProps = (state, { windowId }) => ({
  ...
  panels: getWindowConfig(state, { windowId }).rightPanels,
  rightSideBarPanel: (
    getCompanionWindowsForPosition(state, { position: 'right', windowId })[0] ||
    {}
  ).content,
});
...
const enhance = compose(
  ...
  withPlugins('WindowRightSideBarButtons')
);

export default enhance(WindowRightSideBarButtons);
